...
const TutorialOverlay: React.FC<TutorialOverlayProps> = ({ isOpen, onClose, language }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl p-8 md:p-12 text-center overflow-y-auto max-h-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-left">
          {steps.map((step, i) => (
            <div key={i} className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700/50 hover:border-cyan-500/30 transition-colors">
        ...
        <button
          onClick={onClose}
          className="group relative inline-flex items-center justify-center px-10 py-4 font-bold text-white transition-all duration-200 bg-cyan-600 font-pj rounded-2xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-600 hover:bg-cyan-500 active:scale-95 shadow-[0_0_20px_rgba(8,145,178,0.3)]"
        >
