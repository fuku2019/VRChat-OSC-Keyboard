...
fn init_error_message(err: vr::EVRInitError) -> String {
...
struct OverlayHandle(u64);

impl OverlayHandle {
    fn from_i64(value: i64) -> napi::Result<Self> {
        if value < 0 {
    ...
    fn to_i64(self) -> napi::Result<i64> {
        i64::try_from(self.0)
            .map_err(|_| napi::Error::from_reason("Overlay handle exceeds i64 range"))
    }
}

fn overlay_handle(handle: i64) -> napi::Result<OverlayHandle> {
...
fn row_pitch_bytes(width: u32) -> napi::Result<usize> {
...
fn validate_matrix(matrix: &[f64], name: &str) -> napi::Result<()> {
    if matrix.len() != 16 {
        return Err(napi::Error::from_reason(format!(
...
impl OverlayManager {
    ...
    pub fn set_overlay_texture_d3d11(
        ...
        let overlay_ptr = self.context.overlay;
        let set_texture_fn = {
        ...
        unsafe {
            ...
            let resource: ID3D11Resource = texture
                ...
                .map_err(|e| napi::Error::from_reason(format!(
                    "Texture cast to ID3D11Resource failed: {:?}",
                    e
                )))?;
